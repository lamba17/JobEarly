...
- Score should reflect overall quality (A=excellent, B=good, C=fair, D=poor, F=critical issues)`
    ...
    const result = {
      ...
      criticalCount: report.issues.filter((i: any) => i.category === 'critical').length,
      optionalCount: report.issues.filter((i: any) => i.category === 'optional').length,
    }
...
app.post('/api/enrich-company-names', async (req: Request, res: Response) => {
  try {
...
app.post('/api/generate-cover-letter', async (req: Request, res: Response) => {
  try {
    ...
    }[tone as 'formal' | 'friendly' | 'bold'] ?? 'professional and polished'
